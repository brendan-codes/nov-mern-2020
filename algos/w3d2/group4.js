...
function getMaxServings(recipe, ing) {
  if (ing.length <1){
    return 0
  }
  let counter = 0;
  let copies = {...ing}
  const searchKeys = Object.keys(recipe);
  while(copies[searchKeys[0]] >= recipe[searchKeys[0]]){
    for (const key of searchKeys){
      if(!ing.hasOwnProperty(key)){
        return 0
      }
      if(copies[key] >= recipe[key]){
        copies[key]-=recipe[key]
      }else if(copies[key] < recipe[key]){
        return counter
      }
    }
    counter++
...
function getMaxServings(recipe, ing) {
  ...
  recipeKeys.map(recVal => {ingKeys.map(ingVal => {
      if(recVal == ingVal)
      {
          console.log(recVal);
      }
