...
//   var recipekey=Object.keys(recipe);
...
//     else
//     {
//       if(Math.floor(ingredients[key]/recipe[key])<=maxserving)
//         maxserving=Math.floor([key]/recipe[key]);
//     }
//     if(maxserving==0)
//       return 0;
//   }
...
function getMaxServings(recipe, ingredients) {
  var recipekey=Object.keys(recipe);
  var maxserving=0;
  if (!ingredients.hasOwnProperty(recipekey[0])) {
    return 0;
  }
  else
    maxserving=Math.floor(ingredients[recipekey[0]]/recipe[recipekey[0]])
  ...
  for (key of recipekey) {
    ...
    {
      if(ingredients[key]/recipe[key]<maxserving)
        maxserving=Math.floor(ingredients[key]/recipe[key]);
    }
    if(maxserving==0)
      return 0;
  }
